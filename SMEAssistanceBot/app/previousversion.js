...
try {
 ...
 // The response is a string, so we need to parse it
 const parsedResponse = JSON.parse(updateticket);
 console.log('parsedResponse',parsedResponse);
 
 // The actual ticket data is often inside a 'statusMessage' field
 // which is itself a stringified JSON object. We parse it again.
 let ticketData;
 if (typeof parsedResponse.response === 'string') 
   {
     ticketData = JSON.parse(parsedResponse.response).statusMessage;
 }
  else {
     ticketData = parsedResponse.response.statusMessage;
 }

 // Now we can access the departmentId and assigneeId
 const fetchedDepartmentId = ticketData?.departmentId;
 const fetchedAssigneeId = ticketData?.assigneeId;

 console.log("✅ Ticket assignee updated successfully.");
 console.log("    Fetched department ID:", fetchedDepartmentId);
 console.log("    Fetched assignee ID:", fetchedAssigneeId);
